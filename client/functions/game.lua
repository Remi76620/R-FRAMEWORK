...
function rm.Function.toggleNpcDrops(state, playerId)
    local weaponPickups = {
    ...
    rm.Io.Trace("(rm.Function.toggleNpcDrops) Npc drops: ", state)
end
...
function rm.Function.toggleNpcHealthRegeneration(state, playerId)
    if state then
        SetPlayerHealthRechargeMultiplier(playerId, 1.0)
    else
        SetPlayerHealthRechargeMultiplier(playerId, 0.0)
    end
    rm.Io.Trace("(rm.Function.toggleNpcHealthRegeneration) Npc health regeneration: ", state)
end
...
function rm.Function.toggleDefaultWantedLevel(state, playerId)
    if state then
        SetMaxWantedLevel(5)
    else
    ...
    rm.Io.Trace("(rm.Function.toggleDefaultWantedLevel) Default wanted level: ", state)
end

--- Disables police dispatch for the player.
--- @param playerId number The ID of the player to disable dispatch for.
function rm.Function.toggleDispatchService(state, playerId)
    SetDispatchCopsForPlayer(playerId, state)
    rm.Io.Trace("(rm.Function.toggleDispatchService) Dispatch service: ", state)
end

--- Disables various world scenarios.
function rm.Function.toggleScenarios(state)
    local scenarios = {
    ...
    rm.Io.Trace("(rm.Function.toggleScenarios) Scenarios: ", state)
end

--- Toggle PVP
--- @param state boolean The state to set PVP to
function rm.Function.togglePvp(state)
    SetCanAttackFriendly(PlayerPedId(), state, false)
    NetworkSetFriendlyFireOption(state)
    rm.Io.Trace("(rm.Function.togglePvp) Pvp: ", state)
end