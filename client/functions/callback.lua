...
function rm.TriggerServerCallback(cbName, func, ...)
    if not validateCallbackArguments(cbName, func) then
        rm.Io.Error("Invalid arguments to TriggerServerCallback.")
        return
    end

    registeredCallbacks[requestId] = func

    TriggerServerEvent("rm-framework:triggerServerCallback", cbName, requestId, GetInvokingResource() or "unknown", ...)

    requestId = (requestId + 1) % 2^16
end

RegisterNetEvent("rm-framework:serverCallback", function(RequestId, invoker, ...)
    local callback = registeredCallbacks[RequestId]
    if not callback then
        rm.Io.Error("Callback not registered: " .. RequestId)
        return
    end
