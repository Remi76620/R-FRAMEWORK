...
local function logEvent(action, name, context)
    rm.Io.Trace(("%s %s %s"):format(action, name, context))
end

--- Warn if an event is not registered
--- @param name string The event name
local function warnEvent(name)
    rm.Io.Warn(("Event %s not registered"):format(name))
end

--- Register a new event
--- @param name string The event name
--- @param cb function The callback function
function rm.Event.Register(name, cb)
    if not name or type(cb) ~= "function" then
        rm.Io.Warn("Invalid event name or callback function")
        return
    end
    if not registeredEvents[name] then
        ...
        rm.Io.Warn(("Event %s already registered"):format(name))
    end
end
...
function rm.Event.Trigger(name, ...)
...
if IsDuplicityVersion() then
    ...
    --- @vararg any The event arguments
    function rm.Event.TriggerClient(name, playerId, ...)
    ...
    --- @param name string The event name
    --- @vararg any The event arguments
    function rm.Event.TriggerServer(name, ...)
